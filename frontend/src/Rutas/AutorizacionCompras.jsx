...
const CompraAutorizacion = () => {
  ...
  const handleSeleccionProveedor = (detalleId, proveedor, precio) => {
    ...
    axios.post('/Compras/enviarPreciosProveedor', {
      detalle: { id_detalle: detalleId },
  ...
  return (
    <div>
      ...
      {detalles.map((detalle) => (
        <div key={detalle.id_detalle} className="detalle-compra">
          <p>ID Detalle: {detalle.id_detalle}</p>
          <p>Artículo: {detalle.articulo}</p>
          <p>Cantidad: {detalle.cantidad}</p>
          <p>Observación: {detalle.observacion}</p>
          <div>
            {detalle.proveedores.map((proveedor, index) => (
              <div key={index} className="proveedor-precio">
